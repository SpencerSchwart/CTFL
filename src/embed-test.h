...
void printx (int k, int l, Point point, coord n, coord p, int i, int j, double d[], double y1, double v[])
{
    fprintf (stderr, "|| %d %d %g %g %g n.x=%g n.y=%g p.x=%g p.y=%g i=%d j=%d d[%d]=%g y1=%g v[%d]=%g\n",
            k, l , x, y, cs[], n.x, n.y, p.x, p.y, i, j, l, d[l], y1, l, v[l]);
}

void printy (int k, Point point, coord n, coord p, double gradient)
{
    fprintf (stderr, "|| %d %g %g %g n.x=%g n.y=%g p.x=%g p.y=%g grad=%g\n",
             k, x, y, cs[], n.x, n.y, p.x, p.y, gradient);
}

foreach_dimension()
static inline double dirichlet_gradiento_x (Point point, scalar s, scalar cs,
					   coord n, coord p, double bc,
					   double * coef, bool print)
{
  foreach_dimension()
    n.x = - n.x;
  ...
  if (defined)
    for (int l = 0; l <= 1; l++) {
	  ...
	  cs[i,j-1] && cs[i,j] && cs[i,j+1]) {
        ...
        if (print)
            printx (1, l, point, n, p, i, j, d, y1, v);
    }
#else // dimension == 3
...
double dirichlet_gradiento (Point point, scalar s, scalar cs,
			   coord n, coord p, double bc, double * coef, bool print)
{
#if dimension == 2
  foreach_dimension()
    if (fabs(n.x) >= fabs(n.y))
      return dirichlet_gradiento_x (point, s, cs, n, p, bc, coef, print);
#else // dimension == 3
  if (fabs(n.x) >= fabs(n.y)) {
    ...
    return dirichlet_gradiento_y (point, s, cs, n, p, bc, coef, print);
  return dirichlet_gradiento_z (point, s, cs, n, p, bc, coef, print);
#endif // dimension == 3
  return nodata;
}
...
static inline
coord embed_gradiento (Point point, vector u, coord p, coord n, bool print)
{
  coord dudn;
  foreach_dimension() {
    bool dirichlet = true;
    ...
    if (dirichlet) {
      double val;
      dudn.x = dirichlet_gradiento (point, u.x, cs, n, p, vb, &val, print);
    }
    else // Neumann
      dudn.x = vb;
    if (dudn.x == nodata)
...
void embed_force (scalar p, vector u, face vector mu, coord * Fp, coord * Fmu)
{
  ...
  foreach (reduction(+:Fps) reduction(+:Fmus), nowarning)
	...
	*/
	...
	assert (dimension == 2);
    counter++;
	coord dudn = embed_gradiento (point, u, b, n, true);
    fprintf (stderr, "%d %g %g %g %g %g\n", counter, x, y, cs[], dudn.x, dudn.y);
	foreach_dimension()
...
double embed_vorticity (Point point, vector u, coord p, coord n)
{
  ...
  coord dudn = embed_gradiento (point, u, p, n, false);
...
double embed_flux (Point point, scalar s, face vector mu, double * val)
{
  ...
  *val = 0.;
  ...
  if (dirichlet) {
    normalize (&n);
    bool print = false;
    grad = dirichlet_gradiento (point, s, cs, n, p, grad, &coef, print);
  }
