...
event movie (t += 0.25; t <= t_end) {
  if (t >= 50) {
  scalar omega[];
  vorticity (u, omega);
  char name[80];

  sprintf (name, "vortiso-%g.png", t);
  view (fov = 3, tx = -0.325, ty = -0.50,
        width = 1000, height = 500); 
  clear();
  isoline ("omega", n = 15, min = -5, max = 5);
  draw_vof ("vof", "sf", lw = 5);
  squares ("omega", map = blue_white_red, min = -5, max = 5);
  save (name);
