...
static double x3 = 4.7750;
static double y3 = 10.0850;
...
static void printy (Point point, int j, int i) {
  if (x > x3*0.9999 && x < x3*1.0001 && y > y3*0.9999 && y < y3*1.0001) {
    fprintf (stderr, "||  %d %d x=%g y=%g u.x=%g u.y=%g\n", i, j, x, y, u.x[], u.y[]);
  }
}
