...
#line 1 "cylindernew-backup.c"
#include "embed.h"
#undef EMBED
#include "navier-stokes/centered.h"
#include "../immersed-test.h" 
...
event logfile (i++){
 ...
 fprintf (stderr, "%d %g %d %d %d %d %d %g %g %g %g %g\n",
	   i, t, Re, mgp.i, mgp.nrelax, mgu.i, mgu.nrelax, area, CD, CL, Fmu.x, Fmu.y);
}
...
event profile (t = t_end) {
  ...
  foreach() {
    if (vof[] > 0 && vof[] < 1) {
      ...
      fprintf (fv5, "%g %g %g %g %g %g %g\n", theta, mag, cp, markerForce.x[], markerForce.y[],
