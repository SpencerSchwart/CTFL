...
event moving_cylinder (i++) {
  ...
  vc.y = A*2*M_PI*freq*sin(2*M_PI*freq*(t + dt)); // + dt or not?
...
event logfile (i++, t <= t_end){
  ...
  immersed_forcev2 (vof, p, u, mu, &Fp, &Fmu);
...
*/       
  ...
  fprintf (stderr, "%d %g %d %g %g %g %g %g %g %g %g %g %g\n",
           i, t, Re, CD, avgCD/(count + 1e-6), CL, avgCL/(count + 1e-6), 
           Fp.x, Fp.y, Fmu.x, Fmu.y, xc.y, vc.y);
}
