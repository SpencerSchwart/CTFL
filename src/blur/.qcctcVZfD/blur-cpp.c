...
int main (int argc, char * argv[]) {
  ...
  for (i = 1; i <= 2; i++) {
    const scalar cc[] = theta0[i]*pi/180;
    contact_angle = cc;
    run();
  }
}
...
event interface (t += 0.3125) {
  char name[80];
  sprintf (name, "%d-%d-int-%g", i, j, t);
...
event movie (t += 0.05; t <= t_end) {
  ...
  char name[80];
  sprintf (name, "%d-%d-vof-%g.png",i, j, t);
  FILE * fp1 = fopen (name, "w");
  view (fov = 7, width = 1200, height = 600);
  draw_vof ("cs", "fs", filled = -1);
  draw_vof ("f");
