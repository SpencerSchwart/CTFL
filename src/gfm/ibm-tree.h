...
For the ibmded fraction field *vof*, the function below is modelled
closely on the volume fraction refinement function
[fraction_refine()](fractions.h#fraction_refine). */

static void ibm_fraction_refine (Point point, scalar vof)
{
  double cc = vof[];
  ...
  if (cc <= 0. || cc >= 1.) {
    ...
    coord n = facet_normal (point, vof, sf);
    ...
    foreach_child() {
      ...
      foreach_dimension()
	nc.x = child.x*n.x;
      vof[] = rectangle_fraction (nc, alpha, a, b);
    }
  }
}
...
static void ibm_face_fraction_refine_x (Point point, scalar s)
{
  ...
  if (vof[] <= 0. || vof[] >= 1.) {
    ...
    coord n = facet_normal (point, vof, fs);
    ...
    if (2.*fabs(alpha) < fabs(n.y)) {
      double yc = alpha/n.y;
      int i = yc > 0.;
      fine(sf.x,1,1 - i) = n.y < 0. ? 1. - i : i;
      fine(sf.x,1,i) = n.y < 0. ? i - 2.*yc : 1. - i + 2.*yc;
    }
    else
      fine(sf.x,1,0) = fine(sf.x,1,1) = alpha > 0.;
    ...
    for (int j = 0; j <= 1; j++)
      for (int k = 0; k <= 1; k++)
	if (!fine(vof,0,j,k) || !fine(vof,1,j,k))
	  fine(sf.x,1,j,k) = 0.;
	else {
	  static const coord a = {0.,0.,0.}, b = {.5,.5,.5};
	  coord nc;
	  nc.x = 0., nc.y = (2.*j - 1.)*n.y, nc.z = (2.*k - 1.)*n.z;
	  fine(sf.x,1,j,k) = rectangle_fraction (nc, alpha, a, b);
	}
	...
	if (!is_refined(neighbor(2*i-1))) {
	  if (sf.x[i] <= 0. || sf.x[i] >= 1.)
	    ...
	    double a = sf.y[0,1] <= 0. || sf.y[2*i-1,1] <= 0. ||
	      sf.y[] >= 1. || sf.y[2*i-1] >= 1.;
	    if ((2.*a - 1)*(sf.x[i] - 0.5) > 0.) {
	      fine(sf.x,2*i,0) = a;
	      fine(sf.x,2*i,1) = 2.*sf.x[i] - a;
	    }
	    else {
	      fine(sf.x,2*i,0) = 2.*sf.x[i] + a - 1.;
	      fine(sf.x,2*i,1) = 1. - a;
	    }
	...
	for (int j = 0; j <= 1; j++)
	  ...
	  for (int k = 0; k <= 1; k++)
	#endif
	    if (fine(sf.x,2*i,j,k) && !fine(vof,i,j,k))
	      fine(sf.x,2*i,j,k) = 0.;
      }
  }
}
...
{  
  ...
  for (int i = 0; i <= 1; i++)
    ...
    for (int j = 0; j <= 1; j++)
#endif
      if (fine(vof,0,i,j) && fine(vof,1,!i,!j))
...
static inline void refine_ibm_linear (Point point, scalar s)
{
  foreach_child() {
...
#if dimension == 2
      if (coarse(sf.x,i) && coarse(sf.y,0,j) &&
	  (coarse(vof) == 1. || coarse(vof,child.x) == 1. ||
	   coarse(vof,0,child.y) == 1. || coarse(vof,child.x,child.y) == 1.)) {
	assert (coarse(vof,child.x) && coarse(vof,0,child.y));
	if (coarse(sf.x,i,child.y) && coarse(sf.y,child.x,j)) {
	  // bilinear interpolation
	  assert (coarse(vof,child.x,child.y));
	  s[] = (9.*coarse(s) + 
      ...
      else if (coarse(vof,child.x,child.y) &&
	       ((coarse(sf.x,i) && coarse(sf.y,child.x,j)) ||
		(coarse(sf.y,0,j) && coarse(sf.x,i,child.y)))) {
	// diagonal interpolation
	s[] = (3.*coarse(s) + coarse(s,child.x,child.y))/4.;
      }
#else // dimension == 3
      int k = (child.z + 1)/2;
      if (coarse(sf.x,i) > 0.25 && coarse(sf.y,0,j) > 0.25 &&
	  coarse(fs.z,0,0,k) > 0.25 &&
	  (coarse(vof) == 1. || coarse(vof,child.x) == 1. ||
	   coarse(vof,0,child.y) == 1. || coarse(vof,child.x,child.y) == 1. ||
	   coarse(vof,0,0,child.z) == 1. || coarse(vof,child.x,0,child.z) == 1. ||
	   coarse(vof,0,child.y,child.z) == 1. ||
	   coarse(vof,child.x,child.y,child.z) == 1.)) {
	assert (coarse(vof,child.x) && coarse(vof,0,child.y) &&
		coarse(vof,0,0,child.z));
	if (coarse(sf.x,i,child.y) && coarse(sf.y,child.x,j) &&
	    coarse(fs.z,child.x,child.y,k) &&
	    coarse(fs.z,child.x,0,k) && coarse(fs.z,0,child.y,k)) {
	  assert (coarse(vof,child.x,child.y) && coarse(vof,child.x,0,child.z) &&
		  coarse(vof,0,child.y,child.z) &&
		  coarse(vof,child.x,child.y,child.z));
	  // bilinear interpolation
	  s[] = (27.*coarse(s) + 
      ...
      else if (coarse(vof,child.x,child.y,child.z) &&
	       ((coarse(fs.z,child.x,child.y,k) &&
		 ((coarse(sf.x,i) && coarse(sf.y,child.x,j)) ||
		  (coarse(sf.y,0,j) && coarse(sf.x,i,child.y))))
		||
		(coarse(fs.z,0,0,k) &&
		 ((coarse(sf.x,i,0,child.z) && coarse(sf.y,child.x,j,child.z)) ||
		  (coarse(sf.y,0,j,child.z) && coarse(sf.x,i,child.y,child.z))))
		||
		(coarse(fs.z,child.x,0,k) &&
		 coarse(sf.x,i) && coarse(sf.y,child.x,j,child.z))
		||
		(coarse(fs.z,0,child.y,k) &&
		 coarse(sf.y,0,j) && coarse(sf.x,i,child.y,child.z))
		))
	// diagonal interpolation
	...
	s[] = coarse(s);
	foreach_dimension() {
	  if (coarse(sf.x,(child.x + 1)/2) && coarse(vof,child.x))
	    s[] += (coarse(s,child.x) - coarse(s))/4.;
	  else if (coarse(sf.x,(- child.x + 1)/2) && coarse(vof,- child.x))
	    s[] -= (coarse(s,- child.x) - coarse(s))/4.;
	}
      }
    }
...
void refine_ibm_face_x (Point point, scalar s)
{
  ...
  for (int i = 0; i <= 1; i++)
    if (neighbor(2*i - 1).neighbors &&
	(is_local(cell) || is_local(neighbor(2*i - 1)))) {
      double g1 = sf.x[i] >= 1. && sf.x[i,+1] && sf.x[i,-1] ?
	(v.x[i,+1]/sf.x[i,+1] - v.x[i,-1]/sf.x[i,-1])/8. : 0.;
      double g2 = sf.x[i] >= 1. && sf.x[i,0,+1] && sf.x[i,0,-1] ?
	(v.x[i,0,+1]/sf.x[i,0,+1] - v.x[i,0,-1]/sf.x[i,0,-1])/8. : 0.;
      for (int j = 0; j <= 1; j++)
	for (int k = 0; k <= 1; k++)
	  fine(v.x,2*i,j,k) = sf.x[i] ?
	    fine(sf.x,2*i,j,k)*(v.x[i]/sf.x[i] +
				(2*j - 1)*g1 + (2*k - 1)*g2) : 0.;
    }
  if (is_local(cell)) {
    double g1 = (sf.x[0,+1] + sf.x[1,+1]) && (sf.x[0,-1] + sf.x[1,-1]) ?
      ((v.x[0,+1] + v.x[1,+1])/(sf.x[0,+1] + sf.x[1,+1]) -
       (v.x[0,-1] + v.x[1,-1])/(sf.x[0,-1] + sf.x[1,-1]))/8. : 0.;
    double g2 = (sf.x[1,0,+1] + sf.x[0,0,+1]) && (sf.x[1,0,-1] + sf.x[0,0,-1]) ?
      ((v.x[0,0,+1] + v.x[1,0,+1])/(sf.x[1,0,+1] + sf.x[0,0,+1]) -
       (v.x[0,0,-1] + v.x[1,0,-1])/(sf.x[1,0,-1] + sf.x[0,0,-1]))/8. : 0.;
    for (int j = 0; j <= 1; j++)
      for (int k = 0; k <= 1; k++)
	fine(v.x,1,j,k) = sf.x[] + sf.x[1] ?
	  fine(sf.x,1,j,k)*((v.x[] + v.x[1])/(sf.x[] + sf.x[1]) +
			    (2*j - 1)*g1 + (2*k - 1)*g2) : 0.;
  }
}
